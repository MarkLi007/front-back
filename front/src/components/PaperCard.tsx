...
import { FileText, Calendar, User, ExternalLink, Clock, Tag, Award } from "lucide-react";
    ...
    <div className="paper-card hover:shadow-lg transition-shadow">
      ...
      <div className="mt-4 space-y-2">
        ...
        {citationCount !== undefined && (
          <div className="flex items-center text-sm text-gray-600">
            <Award className="h-4 w-4 mr-2 text-paper-secondary" />
            <span>被引用次数：{citationCount}</span>
          </div>
        )}

        {keywords && keywords.length > 0 && (
          <div className="flex items-center text-sm text-gray-600 flex-wrap gap-1">
            <Tag className="h-4 w-4 mr-2 text-paper-secondary shrink-0" />
        ...
        {license && (
          <div className="flex items-center text-sm text-gray-600">
            <FileText className="h-4 w-4 mr-2 text-paper-secondary" />
