...
export default function AddVersion() {
  ...
  async function loadPaper() {
    ...
    try {
      ...
      try {
        // Try to use the new getPaperInfo function with extended return values
        const [owner, title, author, statusBn, versionCount, reviewCount, reportCount, keywords, license] = await contract.getPaperInfo(id);
        ...
        console.error("Error using new getPaperInfo, falling back to old version:", error);
        
        // Fall back to old contract's getPaperInfo function
        const [owner, title, author, statusBn, versionCount] = await contract.getPaperInfo(id);
  ...
  return (
    <Layout>
      <div className="max-w-3xl mx-auto">
        ...
        {paper.license && (
          <div className="mb-6">
            <span className="text-sm text-gray-500">许可协议: </span>
            <span className="font-medium">{paper.license}</span>
          </div>
        )}
