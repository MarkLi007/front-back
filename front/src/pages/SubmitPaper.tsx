...
import { Upload, AlertCircle, Tag, FileCheck } from "lucide-react";
...
export default function SubmitPaper() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Note: We've removed the AUTHOR role check here since anyone can submit papers now
    ...
    <Layout>
      <div className="max-w-3xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="paper-form paper-card">
          ...
          <div className="form-group">
            <label htmlFor="keywords">关键词 (以逗号分隔)</label>
            <Input
              id="keywords"
              value={keywords}
              onChange={(e) => setKeywords(e.target.value)}
              placeholder="例如: 区块链,Web3,论文"
              disabled={isSubmitting}
            />
            <p className="text-xs text-gray-500 mt-1">多个关键词请用逗号分隔</p>
          </div>
          
          <div className="form-group">
            <label htmlFor="license">许可协议</label>
            <select
              id="license"
              className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-paper-primary"
              value={license}
            ...
            >
              ...
              <option value="CC BY-ND 4.0">Creative Commons BY-ND 4.0</option>
              <option value="CC BY-NC-SA 4.0">Creative Commons BY-NC-SA 4.0</option>
              <option value="CC BY-NC-ND 4.0">Creative Commons BY-NC-ND 4.0</option>
